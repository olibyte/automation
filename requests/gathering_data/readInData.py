...
timing_data = []
with open('requests\gathering_data\TestTimingData.csv') as csv_file:
    file_reader = csv.reader(csv_file)
    for row in file_reader:
        timing_data.append(row) #split each row at commas and make into list of lists
...
for row in timing_data[1:]:
    test_name = row[0]
    if not row[1] or not row[2]: #if either items are blank
        continue
...
for row in column_chart_data[1:]:
    chart_data_str += '%s,\n'%row #substitute, new line, insert each row into string

completed_html = html_string.substitute(labels=column_chart_data[0],
data=chart_data_str) #substitute labels and data

with open('requests/gathering_data/column_chart.html','w') as f:
